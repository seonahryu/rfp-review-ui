...
export function Console() {
  ...
  const canGenerate = Boolean(response?.workflow_gates?.can_generate_recommendations) && allConfirmed
  ...
  const toRecommendation = useCallback(() => {
    if (!allConfirmed) {
      toast.error("확인하지 않은 항목이 있습니다. 모든 항목을 확인해 주세요.")
      return
    }
    if (!response?.workflow_gates?.can_generate_recommendations) {
      toast.error("권고 문장 생성 조건이 충족되지 않았습니다.")
      return
    }
    advanceTo("recommendation")
  }, [allConfirmed, response, advanceTo])

  const confirmRecommendation = useCallback(() => {
    if (!allComplete) {
      toast.error("모든 항목의 판단이 완료되지 않아 최종 결과로 이동할 수 없습니다.")
      return
    }
    setRecommendationConfirmed(true)
    advanceTo("final")
    toast.success("권고 문장 생성 최종 컨펌 완료")
  }, [allComplete, advanceTo])

  const confirmFinal = useCallback(() => {
    setFinalConfirmed(true)
    toast.success("최종 결과 확인 완료")
  }, [])
